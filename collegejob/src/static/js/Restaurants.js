    ...
    constructor(props) {
        ...
        const intersectionCallback = (entries) => {
            ...
            if (entries.length === 1 && this.state.isAutoScrolling === false) {
                if (entries[0].isIntersecting === false && entries[0].boundingClientRect.bottom < 0) {
                    ...
                    let hideSectionIndex = this.state.categories.findIndex((category) => category.category === hideSectionId);
    ...
    async fetchData() {
        ...
        fetch("https://mocki.io/v1/3fb1488d-bbdb-4ddd-9a03-a0d2efc98597")
            ...
            .then((response) => {
                ...
                let allRestaurants = [];    //for restaurantList
                let exclusiveList = [];    // for all isExlusive is true
                let visibleItems = {};       //for left nav links category
    ...
    render() {
        ...
        return (
            <div className="page-wrap" >
                < Sidebar categories={categories} activeCategory={activeCategory} allRestaurants={allRestaurants}
                    onClickCategory={
                                ...
                                return (<
                                    ...
                                    <div className="list-row" > {
                                        category.restaurantList
                                            ...
                                            .map((item, i) => {
                                                return (
                                                    <Item data={item} index={i} moreCount={i + 1 === this.state.visibleItems[category.category] ? category.restaurantList.length - this.state.visibleItems[category.category] : 0}
                                                        onShowMore={() => this.updateVisibleItems(category.category)}
